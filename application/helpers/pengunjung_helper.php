...
function api_ongkir($kotaasal,$kotatujuan,$beratbelanja,$ekspedisi)
{
  ...
  //konversi ke array
		$data=json_decode($response,TRUE);
  //dapatkan kota tujuan
		if (!empty($data['rajaongkir']['destination_details'])) 
		{
			$ongkir['kotatujuan']=$data['rajaongkir']['destination_details'];	
		}

		if (!empty($data['rajaongkir']['results']))
		 {
		//dapat pilihan paket yang tersedia
		$ongkir['paketongkir']=$data['rajaongkir']['results']['0']['costs'];
		return $ongkir;
		}
